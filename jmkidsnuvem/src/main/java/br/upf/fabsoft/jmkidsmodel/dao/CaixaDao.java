...
public class CaixaDao extends GenericDao<Caixa> {
	...
	/*
	 * public List<CaixaMovimento> getCaixaMovimentoList(Caixa caixa) throws
	 * Exception { EntityManager em = null; try { em = JpaUtil.getEntityManager();
	 * String oql =
	 * "select c from CaixaMovimento c where c.brinquedoOperacao.caixa.quiosque.id = "
	 * +caixa.getQuiosque().getId()+" and c.caixa.id = "+caixa.getId();
	 * List<CaixaMovimento> ret = em.createQuery(oql).getResultList(); return ret; }
	 * catch (Exception e) { e.printStackTrace(); throw e; } finally { if (em !=
	 * null) em.close(); } }
	 * 
	 * public List<CaixaMovimentoDois> getCaixaMovimentoDoisList(Caixa caixa) throws
	 * Exception { EntityManager em = null; try { em = JpaUtil.getEntityManager();
	 * String oql =
	 * "select c from CaixaMovimentoDois c where c.brinquedoOperacao.caixa.quiosque.id = "
	 * +caixa.getQuiosque().getId()+" and c.caixa.id = "+caixa.getId(); return
	 * em.createQuery(oql).getResultList(); } catch (Exception e) {
	 * e.printStackTrace(); throw e; } finally { if (em != null) em.close(); } }
	 */
